...
def read_text_file(file_path):
    """
    以二进制方式读取文件并自动检测编码，然后用检测到的编码读取文本内容。
    如果检测不到编码，则使用默认的 utf-8，同时忽略解码错误。
    """
    with open(file_path, 'rb') as f:  # 以二进制方式读取
        raw_data = f.read()
...
def convert(subscription,target,other_config={}):
    ...
    if subscription[:8] == 'https://':
        ...
        try:
            content = read_text_file(subscription)
...
def subconverterhandler(subscription,input_config={'target':'transfer','rename':'','include':'','exclude':'','config':''}):
    ...
    origin_configparse = ConfigParser(interpolation=None)
    origin_configparse.read('./generate.ini',encoding='utf-8')
    origin_config = {
        'url': origin_configparse[target]['url'],
        'rename': origin_configparse[target]['rename'],
        'include': origin_configparse[target]['include'],
...
def deduplicate(clash_provider,keep_nodes=1): # Proxies deduplicate. If proxies with the same servers are greater than keep_nodes, they will not be added.
    ...
    for proxy in proxies:
        ...
        #print(f'subconvert.py----->{server}')
        # 处理 None 类型
        if server is None:
            ip = "127.0.0.1"  # 可根据需求设置默认值，例如 "127.0.0.1"
        elif isinstance(server, int):  
            ip = str(server)  # 将整数转换为字符串
        elif isinstance(server, str) and server.replace('.', '').isdigit():
            ip = server  # 字符串形式的 IP 地址
        else:
            try:
                ip = socket.gethostbyname(server)  # 解析域名为 IP
            except Exception:
                ip = server  # 如果解析失败，保留原始值
        #print(f'subconvert.py----->{server}----->{ip}')
        
        # 可以在这里将 ip 加入 servers 字典或其他需要的处理逻辑
        if ip in servers:
